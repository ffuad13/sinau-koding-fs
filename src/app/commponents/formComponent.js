  ...
  const hadleSubmit = (event) => {
    event.preventDefault();
		...
		if (!email) {
			...
			isValid = false
		} else if (!/@(google\.com|yahoo\.com)$/.test(email)) { //regex
      ...
      <div className="mb-4">
        ...
        <label htmlFor="options" className="block text-gray-700 text-sm font-bold mb-2">
          Select an option:
				...
				<select id="options" value={selectedOption} onChange={handleSelectChange} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-amber-50">
			...
			<button type="submit" className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 focus:outline-none focus:shadow-amber-50">Submit</button>
    </form>
  );
}
